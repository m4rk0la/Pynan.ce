...
from .utils import calcula_total, calculo_equilibrio_financeiro
...
def home(request):
    ...
    percentual_gastos_essenciais,percentual_gastos_nao_essenciais = calculo_equilibrio_financeiro()
    return render(request, 'home.html', {'contas': contas, 'saldo_total': saldo_total,'total_entradas':total_entradas,
...
def cadastrar_banco(request) :
    ...
    if len(apelido.strip()) == 0 or len(valor.strip()) == 0 or len(banco.strip()) == 0 or len(tipo.strip()) == 0 or len(icone.strip()) ==0:
...
def cadastrar_categoria(request):
    ...
    if len(nome.strip()) == 0 or len(essencial.strip()) == isinstance :
        messages.add_message(request, constants.ERROR, 'Preencha todos os campos!')
        return redirect('/perfil/gerenciar/')
...
def dashboard(request):
    ...
    for categoria in categorias:
        valores =Valores.objects.filter(categorias=categorias)
        for v in valores:
            total = total + v.valor
